<header class="c-header c-header-light c-header-fixed">
  ...
  <ul class="c-header-nav d-md-down-none">
    ...
    <li class="c-header-nav-item px-3">
      <a class="c-header-nav-link" href="{{ route('frontend.index') }}">@lang('Home')</a>
    </li>
    ...
    <li class="c-header-nav-item dropdown">
      <x-utils.link class="c-header-nav-link" data-toggle="dropdown" href="#" role="button" aria-haspopup="true"
        ...
        <x-slot name="text">
          <div class="c-avatar">
            {{-- muestra foto avatar --}}
            <img class="c-avatar-img" src="/storage/avatars/{{ $logged_in_user->avatar_path }}"
              alt="{{ $logged_in_user->email ?? '' }}">
          </div>
        </x-slot>
      </x-utils.link>
      {{-- sub-menu click en avatar (cuenta) --}}
